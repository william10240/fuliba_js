...
function get_list(list_index) {
    ...
    _request(list_url, (html) => {
        ...
        logger.error("列表页请求失败: 第" + list_index + "页")
    })
}
...
function get_page(page_url, page_title) {
    _request(page_url, (html) => {
        let tag = page_title.match('(.*?)福利汇总第(.*?)期')
        logger.info("--内容页请求成功:" + tag[0])
        const $ = cheerio.load(html);
        ...
        for (const el of els) {
            ...
            // 开始调用 get_page
            get_content(content_url, tag, $(el).text())
        }
    }, () => {
...
// 获取 内容 分页
function get_content(content_url, content_tag, content_index) {
    logger.info("----详情页请求成功:" + content_tag[0] + " 第" + content_index + "页")
    _request(content_url, (html) => {
        ...
        for (const el of els) {
            ...
            if (img_src) {
                let img_path = path.join(content_tag[1], content_tag[2], content_index, path.basename(img_src))
                // 测试查看参数
                ...
                logger.error("src为空:" + content_tag[0] + " 第" + content_index + "页")
            }
        }
    }, () => {
...
function save_img(img_src, img_path) {
    ...
    let abs_path = path.join(IMG_PATH, img_path)
    // 检测文件已近下载过
    ...
    // 检测无后缀名文件已经下载过
    if (glob.sync(abs_path + "*").length > 0) {  // 如果图片是没有后缀名的
        logger.debug("--------图片已下载过:" + img_path)
        return
    }
    
    let img_folder = path.dirname(abs_path)
    // 判断目录是否存在
    ...
    _request(img_src, (res) => {
        ...
        if (!path.extname(img_src)) {
            try {
                ...
                logger.error("--------图片解析失败:" + img_path)
                return
            }
            img_path = img_path + '.' + exts[0]
        }
        
        fs.writeFile(abs_path, res, "binary", err => {
            if (!err) {
                logger.info('--------图片保存成功:' + img_path)
            } else {
                logger.error("--------图片保存失败:" + img_path)
            }
        })
        
    }, () => {
        logger.error("--------图片请求失败:" + img_path)
    }, true)
